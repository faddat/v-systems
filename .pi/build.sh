...
sudo bash -c "echo vsys > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/vsys.img"

# loop-mount the image file so it becomes a disk
export LOOP=$(sudo losetup --find --show images/vsys.img)
